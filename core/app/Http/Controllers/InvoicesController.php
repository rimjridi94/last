   ...
   public function __construct(Invoice $invoice, Product $product, Client $client,  Tax $tax, Currency $currency, InvoiceItem $items, Setting $setting, Number $number, InvoiceSetting $invoiceSetting, Template $template, MailSetting $mail_setting){
    ...
    public function store(InvoiceFromRequest $request)
	{
        ...
        $invoiceData = array(
            ...
            'avoir'        => $request->get('avoir')?$request->get('avoir'):0,
	...
	{
	    $avoir = !empty($_GET['avoir']) ?$_GET['avoir']:null;
        ...
        if($invoice){
            ...
            if ($avoir){
                ...
                return view('invoices.avoir', compact('invoice', 'clients', 'taxes', 'currencies', 'statuses'));
            }
    ...
    public function update(InvoiceFromRequest $request, $uuid)
	{
        ...
        return Response::json(array('échoué' => false, 'msg' =>'Désolé, mise a jour echoué, Réessayez'), 400);
	}
    ...
    public function send($uuid){
        ...
        if($invoice){
            ...
            if($mail_setting){
                ...
                Flash::success('Email a été envoyé');
                }
                catch (\Exception $e){
                ...
                Flash::error('S’il vous plaît définir les détails du serveur de messagerie d’abord dans le réglage> Email');
            }
