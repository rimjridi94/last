...
class BonController extends Controller {
    ...
    public function create()
    {
        ...
        return view('bon.create', compact('invoice_num','products', 'clients','taxes','currencies', 'statuses', 'settings'));
    }
    ...
    public function store(InvoiceFromRequest $request)
    {
        $invoiceData = array(
            ...
            'number'        => 'bon_'.$request->get('number'),
        ...
        if($invoice){
            ...
            return Response::json(array('Succès' => true,'redirectTo'=>route('bon.show', $invoice->uuid), 'msg' =>  'Creation Réussite'), 200);
        }
    ...
    public function show($uuid)
    {
        ...
        if($invoice){
            ...
            return view('bon.show', compact('invoice','settings', 'invoiceSettings'));
        }
        return redirect('bon');
    }
    /**
    ...
    public function edit($uuid,Request $request)
    {
        ...
        if($invoice){
            ...
            $statuses   = statuses();
            $invoice->totals = $this->invoice->invoiceTotals($uuid);
            return view('bon.edit', compact('invoice', 'clients', 'taxes', 'currencies', 'statuses'));
        }
        return redirect('bon');
    }
    ...
    public function update(InvoiceFromRequest $request, $uuid)
    {
        ...
        if($invoice){
            ...
            return Response::json(array('Succès' => true,'redirectTo'=>route('bon.show', $invoice->uuid), 'msg' => 'mise a jour Réussite'), 200);
        }
    ...
    public function deleteItem(){
        ...
        if($this->items->deleteById($id)) {
            return Response::json(array('success' => true, 'msg' => 'Supression Réussite'), 201);
        }
        return Response::json(array('success' => false, 'msg' => 'Désolé, suppression echoué. Réessayez.'), 400);
    }
    ...
    public function invoicePdf($uuid){
        ...
        if($invoice){
            ...
            $invoiceSettings = $this->invoiceSetting
                ...
                ->first();
            $invoice->totals = $this->invoice->invoiceTotals($uuid);
            $pdf = \PDF::loadView('bon.pdf', compact('settings', 'invoice', 'invoiceSettings'));
            return $pdf->download('bon_'.$invoice->number.'_'.date('Y-m-d').'.pdf');
        }
        return redirect('bon');
    }
