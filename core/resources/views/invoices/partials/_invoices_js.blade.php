...
function calcTotals(){
        ...
        $('tr.item').each(function(){
            ...
            var nb_jours    = parseFloat($(this).find("[name='nb_jours']").val());
            var price        = parseFloat($(this).find("[name='price']").val());
            var itemTax     = parseFloat($(this).find("[name='tax']").val());
            var itemTotal   = parseFloat(quantity * price*nb_jours) > 0 ? parseFloat(quantity * price*nb_jours) : 0;
            var taxValue    = $(this).find("[name='tax'] option[value='" + itemTax + "']").attr('data-value');
            subTotal        += parseFloat(price * quantity*nb_jours) > 0 ? parseFloat(price * quantity*nb_jours) : 0;
            totalTax        += parseFloat(price * quantity *nb_jours* itemTax/100) > 0 ? parseFloat(price * quantity*nb_jours * taxValue/100) : 0;
...
function cloneRow(in_tbl_name)
{
    ...
    var strHtml51 = '<div class="form-group">{!! Form::input("number","nb_jours",null, ["class" => "form-control input-sm calcEvent nb_jours", "id"=>"nb_jours", "required","step" => "any", "min" => "1"]) !!}</div> ';
